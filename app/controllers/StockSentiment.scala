...
class StockSentiment @Inject()(ws: WSClient, configuration: Configuration) extends Controller {
  ...
  private def getTextSentiment(text: String): Future[WSResponse] = {
    ...
    logger.info(s"sentimentUrl: text = $sentimentUrl")
    val mapp = ws.url(sentimentUrl) post Map("text" -> Seq(text))
    logger.info(s"ws.url(sentimentUrl) post Map(text -> Seq(text)) = $mapp")
  ...
  private def getAverageSentiment(responses: Seq[WSResponse], label: String): Double = {
    for(response <- responses){
      ...
      logger.info(s"response: = $responseJson")
  ...
  private def getTweets(symbol:String): Future[WSResponse] = {
    ...
    logger.info(s"tweetUrl: $tweetUrl")
    val tweetUrlFormat = tweetUrl.format(symbol)
    logger.info(s"tweetUrl.format(symbol): $tweetUrlFormat")
